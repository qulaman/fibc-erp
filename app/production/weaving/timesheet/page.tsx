...
export default function WeavingTimesheetPage() {
  const [loading, setLoading] = useState(false);

  // Состояние фильтров (Месяц, Цех зафиксирован)
  const [currentDate, setCurrentDate] = useState(new Date());
  const department = 'weaving'; // Зафиксирован для ткачества

  // --- НАСТРОЙКИ "КИСТИ" (Нижняя панель) ---
  ...
  const fetchData = async () => {
    ...
    try {
      // Загружаем сотрудников ткацкого цеха
      const { data: empData, error: empError } = await supabase
        .from('employees')
        .select('id, full_name, role')
        .eq('is_active', true)
        .in('role', ['operator_loom', 'weaver', 'master_weaving', 'operator_weaver'])
        .order('full_name');

      if (empError) throw empError;
      setEmployees(empData || []);

      // Загружаем посещаемость за выбранный месяц
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Табель_Ткачество_${monthName}.csv`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
           <h1 className="text-xl md:text-3xl font-bold flex items-center gap-2">
             <Calendar className="text-[#E60012]" size={20} /> <span className="hidden sm:inline">Табель Ткачества</span><span className="sm:hidden">Табель</span>
           </h1>
               ...
               {!loading && employees.length === 0 && (
                  <tr>
                     <td colSpan={35} className="p-12 text-center text-zinc-500">
                        В ткацком цехе нет сотрудников.
                     </td>
                  </tr>
               )}
            </tbody>
