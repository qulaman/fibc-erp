...
import { Layers, Users, Package, Trash2, Plus, CheckCircle2, Factory, Sun, Moon, User, AlertTriangle } from "lucide-react";
      ...
      for (const roll of processedRolls) {
        ...
        const { data: lamRoll, error: lamRollError } = await supabase
          ...
          .insert({
            ...
            length: 0, // TODO: если нужно метраж
        ...
        for (const dosator of activeDosators) {
          ...
          console.log('✅ Списано сырье:', deductionResult);
        }
      }
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader className="pb-3">
              <CardTitle className="text-white flex items-center gap-2 text-sm"><Layers size={16}/> Дозаторы (Сырье)</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {dosators.map((d, idx) => (
                <div key={idx} className="flex gap-2">
                  <Select onValueChange={v => updateDosator(idx, 'material_id', v)}>
                    <SelectTrigger className="bg-zinc-950 border-zinc-700 text-white text-sm h-9">
                      <SelectValue placeholder={`Дозатор ${idx + 1}...`} />
                    </SelectTrigger>
                    <SelectContent>
                    ...
                    <div key={roll.id} className="p-3 bg-zinc-950 border border-zinc-800 rounded-lg">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <div className="text-xs text-zinc-500 mt-1">→ {roll.output_roll_number}</div>
                        </div>
                        <button
