...
export default function LaminationTimesheetPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Загружаем сотрудников цеха ламинации
      const { data: empData, error: empError } = await supabase
        .from('employees')
        .select('id, full_name, role')
        .eq('is_active', true)
        .in('role', ['operator_lamination', 'master_lamination'])
        .order('full_name');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Табель_Ламинации_${monthName}.csv`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
           <h1 className="text-xl md:text-3xl font-bold flex items-center gap-2">
             <Calendar className="text-[#E60012]" size={20} /> <span className="hidden sm:inline">Табель Ламинации</span><span className="sm:hidden">Табель</span>
           </h1>
               ...
               {!loading && employees.length === 0 && (
                  <tr>
                     <td colSpan={35} className="p-12 text-center text-zinc-500">
                        В цехе ламинации нет сотрудников.
                     </td>
                  </tr>
               )}
            </tbody>
