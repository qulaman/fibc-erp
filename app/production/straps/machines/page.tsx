...
import { Ribbon, CheckCircle2, PlayCircle, PlusCircle, Ruler, Weight, User, Calendar, Clock, RefreshCw, Factory } from 'lucide-react';
...
export default function StrapsMachinesPage() {
      ...
      const machinesStatus: MachineStatus[] = await Promise.all(
        (looms || []).map(async (loom) => {
          ...
          return {
            ...
            name: loom.name,
            code: loom.code || '',
            status: session ? 'busy' : 'free',
            activeSession: session ? {
              id: session.id,
              spec_name: session.spec_name || 'Не указана',
              total_length: session.total_length || 0,
              total_weight: session.total_weight || 0,
              started_at: session.started_at,
              operator_name: (session.employees as any)?.full_name,
            } : undefined,
          };
        })
      );
  ...
  useEffect(() => {
    fetchMachinesStatus();
    const interval = setInterval(fetchMachinesStatus, 60000);
    ...
    <div className="page-container">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-zinc-400">Выберите станок для начала производства</p>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-zinc-500">
          ...
          <p>Станки не найдены. Добавьте оборудование типа "loom_flat" в справочник.</p>
        </div>
      ) : (
          ...
          {machines.map((machine) => (
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {machine.code && <p className="text-xs text-zinc-400">{machine.code}</p>}
                    </div>
                  </div>
                  <div className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 ${
                    machine.status === 'busy' ? 'bg-blue-600 text-white' : 'bg-emerald-600 text-white'
                  }`}>
                    {machine.status === 'busy' ? (
                      <><PlayCircle size={14} /> ЗАНЯТ</>
                    ) : (
              ...
              <div className="p-4 space-y-3">
                {machine.status === 'busy' && machine.activeSession ? (
                  <>
                    {/* Спецификация */}
                    <div className="flex items-center gap-3 p-3 bg-blue-900/30 rounded-lg border border-blue-800/30">
                      <Ribbon size={20} className="text-blue-400 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-blue-300">Тип стропы</div>
                        <div className="text-base font-bold text-white truncate">{machine.activeSession.spec_name}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 bg-zinc-900/50 rounded-lg">
                        ...
                        <div className="text-lg font-bold text-white">{machine.activeSession.total_length} м</div>
                      </div>
                      <div className="p-3 bg-zinc-900/50 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                        ...
                        <div className="text-lg font-bold text-white">{machine.activeSession.total_weight} кг</div>
                      </div>
                    </div>

                    {/* Оператор */}
                    {machine.activeSession.operator_name && (
                      <div className="p-3 bg-zinc-900/50 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                          <User size={14} className="text-zinc-400" />
                          <div className="text-xs text-zinc-400">Оператор</div>
                        </div>
                        <div className="text-sm font-medium text-white">{machine.activeSession.operator_name}</div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 text-xs text-zinc-500">
                      <Calendar size={12} />
                      Начат: {new Date(machine.activeSession.started_at).toLocaleString('ru-RU', {
                  ...
                  <div className="text-center py-8">
                    ...
                    <div className="text-sm text-zinc-400">Готов к запуску производства</div>
                  </div>
                )}
              </div>
              ...
              <div className="px-4 pb-4">
                <Link
                  href={`/production/straps/production?machine_id=${machine.id}`}
                ...
                >
                  {machine.status === 'busy' ? (
                    <span className="flex items-center justify-center gap-2">
                      <PlayCircle size={18} /> Продолжить производство
                    </span>
                  ) : (
                    <span className="flex items-center justify-center gap-2">
                      <PlusCircle size={18} /> Начать производство
                    </span>
                  )}
                </Link>
              </div>
