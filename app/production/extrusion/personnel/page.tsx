...
import { Users, UserPlus, Edit, Save, X, Trash2, Briefcase, UmbrellaIcon, HeartPulse } from "lucide-react";
...
interface Employee {
  ...
  birth_date?: string;
  is_active: boolean;
  work_status?: string; // 'active', 'vacation', 'sick_leave'
  created_at?: string;
}

const EXTRUSION_ROLES = [
  { value: 'operator_extruder', label: 'Оператор Экструзии' },
  { value: 'operator_winder', label: 'Намотчик' },
];

export default function ExtrusionPersonnelPage() {
  ...
  const handleAdd = async () => {
    ...
    if (error) {
      ...
      setFormData({ full_name: '', role: 'operator_extruder', birth_date: '' });
      fetchEmployees();
    }
  };
  ...
  const getRoleLabel = (role: string) => {
    return EXTRUSION_ROLES.find(r => r.value === role)?.label || role;
  };
    ...
    <div className="page-container max-w-[100vw] overflow-x-hidden p-3 md:p-6">
      <div className="mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
          <h1 className="text-xl md:text-3xl font-bold flex items-center gap-2">
            <div className="bg-[#E60012] p-1.5 md:p-2 rounded-lg">
              <Users size={18} className="text-white md:hidden" />
              <Users size={24} className="text-white hidden md:block" />
            </div>
            <span className="hidden sm:inline">Персонал Экструзии</span>
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => { setFormData({ full_name: '', role: 'operator_extruder', birth_date: '' }); setIsDialogOpen(true); }}
        ...
        <p className="text-xs md:text-sm text-zinc-400">Управление сотрудниками цеха экструзии</p>
      </div>
                        ...
                        <tr key={emp.id} className="hover:bg-zinc-800/50 transition-colors">
                          ...
                          <td className="px-3 md:px-4 py-3 hidden sm:table-cell">
                            {editingId === emp.id ? (
                              ...
                              >
                                <SelectTrigger className="bg-zinc-950 border-zinc-700 text-white w-48">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  {EXTRUSION_ROLES.map(role => (
                    ...
                    <tbody className="divide-y divide-zinc-800">
                      {dismissedEmployees.map(emp => (
                        <tr key={emp.id} className="hover:bg-zinc-800/50 transition-colors">
                          ...
                          <td className="px-3 md:px-4 py-3 text-zinc-500 text-sm hidden sm:table-cell">
                            {getRoleLabel(emp.role)}
                          </td>
                          <td className="px-3 md:px-4 py-3">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-zinc-900 text-white border-zinc-800 max-w-[95vw] sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg">Новый сотрудник экструзии</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div>
              <label className="text-sm text-zinc-400">Должность *</label>
