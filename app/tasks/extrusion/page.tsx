...
export default function ExtrusionTasksPage() {
  const { user } = useAuth();
  ...
  const logChange = async (taskId: string, fieldName: string, oldValue: any, newValue: any) => {
    try {
      await supabase
        ...
        .insert([{
          task_id: taskId,
          changed_by: user?.email || 'Цех Экструзии',
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="h1-bold">
              ...
              {' '}Задачи цеха Экструзии
            </h1>
            <p className="page-description">Задачи от руководства для цеха экструзии</p>
          </div>
        </div>
      </div>
