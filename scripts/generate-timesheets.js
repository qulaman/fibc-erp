...
const departments = [
  {
    name: 'lamination',
    title: 'Ламинации',
    roles: ['operator_lamination', 'master_lamination'],
    emptyMsg: 'В цехе ламинации нет сотрудников.'
  },
  {
    name: 'straps',
    title: 'Строп',
    roles: ['strap_operator', 'strap_master'],
    emptyMsg: 'В цехе строп нет сотрудников.'
  },
  {
    name: 'cutting',
    title: 'Кроя',
    roles: ['operator_cutting', 'cutter'],
    emptyMsg: 'В цехе кроя нет сотрудников.'
  },
  {
    name: 'sewing',
    title: 'Пошива',
    roles: ['operator_sewing', 'seamstress'],
    emptyMsg: 'В цехе пошива нет сотрудников.'
  }
];

const fnName = (name) => name.charAt(0).toUpperCase() + name.slice(1);
...
export default function ${fnName(dept.name)}TimesheetPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const { data: empData, error: empError } = await supabase
        ...
        .eq('is_active', true)
        .in('role', ${JSON.stringify(dept.roles)})
        .order('full_name');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", \`Табель_${dept.title}_\${monthName}.csv\`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-xl md:text-3xl font-bold flex items-center gap-2">
            ...
            <span className="hidden sm:inline">Табель ${dept.title}</span><span className="sm:hidden">Табель</span>
          </h1>
...
departments.forEach(dept => {
  const filePath = path.join(__dirname, '..', 'app', 'production', dept.name, 'timesheet', 'page.tsx');
  fs.writeFileSync(filePath, template(dept), 'utf8');
  console.log(`✅ Created: ${filePath}`);
});

console.log(`\n🎉 Сгенерировано ${departments.length} страниц табеля!`);
